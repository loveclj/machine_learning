...
class Train():
    ...
    def trainOneEpoch(self):
        ...
        while n < self.numSamples:
            ...
            cdef  float x = bum[0]
            cdef  float y = bmu[1]
            while somx < self.SOM_X:
                while somy < self.SOM_Y:
                    cdef float deltax = x - somx
                    cdef float deltay = y - somy
                    cdef float dist   = deltax * deltax + deltay * deltay
                    cdef float neigorFactor = 1
